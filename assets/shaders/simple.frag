...
void main() {
	...
	vec3 light_l = lightPos.xyz - worldPos;
	float dist = length(light_l);
	vec3 light_dir = normalize(light_l);
	vec3 attenuation = intensity.xyz / (dist * dist);
	// color += calculatePBRLightContribution( pbrInputs, light_dir,  attenuation);
	// ambient occlusion
